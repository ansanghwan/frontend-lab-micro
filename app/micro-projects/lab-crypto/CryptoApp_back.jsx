...
export default function CryptoApp() {
  ...
  const [usdToKrw, setUsdToKrw] = useState(null);
  const [loadingRate, setLoadingRate] = useState(true);

  const [sortType, setSortType] = useState("market_cap"); // ✅ 정렬 타입 상태
  const isLoading = loadingCrypto || loadingRate;
  ...
  useEffect(() => {
    const fetchCryptos = async () => {
      try {
        ...
        setCryptos(data); // ✅ 전체 저장 후 밑에서 정렬 + 상위 10개 사용
